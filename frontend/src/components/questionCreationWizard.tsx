...
const QuestionCreationWizard: React.FC<{ answererId?: number }> = props => {
    ...
    const createQuestion = () => {
        nextStep();
        questionService.create_question(props.answererId!, questionText, descriptionText).then(res => setResult(res))
    }
        ...
        if (result) {
            ...
            return (
                <Stack spacing={3} direction="row" justifyContent="center" sx={{mt: 5}}>
                    <CircularProgress/>
                    <Typography variant="h5" gutterBottom>正在创建问题</Typography>
                </Stack>
            )
        }
    }
                        ...
                        {props.answererId ? (
                            <>
                                ...
                                <Stack spacing={2} direction="row" sx={{mt: 2}}>
                                    <Button variant="contained" onClick={nextStep}>下一步</Button>
                                    <Button variant="outlined" color="error" component={RouterLink}
                                            to={`/question/select-answerer`}>重新选择</Button>
                                </Stack>
                            </>
                        ) : (
                            <>
                                ...
                                <Button variant="outlined" color="warning" component={RouterLink}
                                        to={`/question/select-answerer`}>浏览列表</Button>
                            </>
                        )}
                    </StepContent>
                </Step>
                <Step>
                    ...
                    <StepContent>
                        ...
                        <TextField label="详细描述"
                                   ...
                                   rows={5}/>
                        <Stack spacing={2} direction="row" sx={{mt: 2}}>
                            <Button variant="contained" onClick={checkInputAndNextStep}>下一步</Button>
                            <Button variant="outlined" color="error" onClick={previousStep}>上一步</Button>
                        </Stack>
                    </StepContent>
                </Step>
                <Step>
                    ...
                    <StepContent>
                        ...
                        <Stack spacing={2} direction="row" sx={{mt: 2}}>
                            <Button variant="contained" onClick={createQuestion}>支付并完成提问</Button>
                            <Button variant="outlined" color="error" onClick={previousStep}>上一步</Button>
                        </Stack>
                    </StepContent>
                </Step>
